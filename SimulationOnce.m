...
%     x = [0 0 0 0 0];
    ...
    for i = 1:size(outerBoundary, 1)
        ...
        if (alfa >= 0 && alfa <= 1) && (betta >= 0 && betta <= 1)
            ...
            isCollision = true;
            pocetNaburani = pocetNaburani + 1;
            plot([A(1) B(1)], [A(2), B(2)], 'Color', 'blue', 'LineWidth', 5);
        end
        ...
        if (alfa1 >= 0 && alfa1 <= 1) && (betta1 >= 0 && betta1 <= 1)
            ...
            isCollision = true;
            pocetNaburani = pocetNaburani + 1;
            plot([D1(1) E1(1)], [D1(2), E1(2)], 'Color', 'green', 'LineWidth', 5);
        end
...
fprintf("pocet naburani: %d\n", pocetNaburani);
fit = fit1 + fit2 + fit3 + fit5 + fit6;
fprintf("fit celkovo: %f\n", fit);
